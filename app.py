...
@app.route("/")
def index():
    return {"message": "BeeWell Backend API is running!", "version": "2.1"}
...
def chat_handler():
    try:
        ...
        logger.info(f"[BeeWell] Session ID: {session_id}")
        ...
        logger.info(f"[BeeWell] Response session ID: {result.get('session_id', 'None')}")
...
def list_sessions():
    ...
    try:
        ...
        for session_id in sessions: 
            if session_id in memory_manager.session_metadata:
                profile = memory_manager.session_metadata[session_id]
                user_id = memory_manager.session_user_mapping.get(session_id, "Unknown")
                session_details.append({
                    "session_id": session_id,
                    "user_name": profile.get('name', 'Unknown'),
                    "user_identifier": user_id,
                    "message_count": len(memory_manager.user_memories[session_id].chat_memory.messages)
                })
...
def clear_session_endpoint(session_id):
    """Debug endpoint to clear a specific session"""
    try:
        from FAISS_rag_pipeline import clear_user_session
